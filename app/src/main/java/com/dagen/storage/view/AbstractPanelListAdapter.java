...
//        lv_content.setFastScrollEnabled(false);
        ...
        RelativeLayout.LayoutParams lp_tv_title =
                new RelativeLayout.LayoutParams(titleWidth, titleHeight);
        ...
        RelativeLayout.LayoutParams lp_mhsv_row =
                new RelativeLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, titleHeight);
...
//        lv_column.setDivider(context.getResources().getDrawable(R.drawable.column_item_divider));
        RelativeLayout.LayoutParams lp_lv_column =
        ...
        mhsv_content.addView(
                lv_content);//因为 lv_content 在 xml 文件中已经设置了 layout 为 match_parent，所以这里add时不需要再加
        // LayoutParameter 对象
        mhsv_content.setOverScrollMode(View.OVER_SCROLL_NEVER);//去除滑动到边缘时出现的阴影
        RelativeLayout.LayoutParams lp_mhsv_content =
        ...
        RelativeLayout.LayoutParams lp_srl =
                new RelativeLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
                        ViewGroup.LayoutParams.MATCH_PARENT);
...
//                ll_contentItem = (LinearLayout) lv_content.getChildAt(lv_content.getFirstVisiblePosition());//获得content的第一个可见item
    ...
    private void initRowLayout() {
        ...
        if (ll_contentItem == null) {
            if (itemWidthList != null) {
                ...
                try {
                    throw new Exception(
                            "how can I set the row width if you don`t give me any reference");
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
        } else {
            View[] contentViews = new View[widthArray.length];
            int j = 0;
            for (int i = 0; i < ll_contentItem.getChildCount(); i++) {
                View view = ll_contentItem.getChildAt(i);
                if (view.getWidth() > 0) {
                    contentViews[j++] = view;
                }
            }
            for (int i = 0; i < widthArray.length; i++) {
                widthArray[i] = contentViews[i].getWidth();
            }
        }
    ...
    private List<String> getRowDataList() {
        if (rowDataList == null) {
            try {
                throw new Exception(
                        "you must set your column data list by calling setColumnDataList(List<String>)");
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    private class HorizontalScrollListener
            implements MyHorizontalScrollView.OnHorizontalScrollListener {
        @Override
        public void onHorizontalScrolled(MyHorizontalScrollView view, int l, int t, int oldl,
                                         int oldt) {
...
//                view.getLayoutParams().height = columnItemHeight;
    ...
    private class DefaultRefreshListener implements SwipeRefreshLayout.OnRefreshListener {
        ...
        public void onRefresh() {
            Toast.makeText(context, "请调用PanelListAdapter的setOnRefreshListener()并传入你的Listener",
