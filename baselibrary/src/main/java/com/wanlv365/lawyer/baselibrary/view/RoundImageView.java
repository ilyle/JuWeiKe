...
public class RoundImageView extends ImageView {
    ...
    private void obtainStyledAttrs(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if(currMode==MODE_CIRCLE) {//只有在圆形时  设置边框有效
            outBorder = (int) a.getDimension(R.styleable.RoundImageView_outBorder, outBorder);
            outBorderColor = a.getColor(R.styleable.RoundImageView_outColor, outBorderColor);
        }
    ...
    private void initViews() {
        ...
        if(currMode==MODE_CIRCLE) {
            ...
            mOuterPaint.setColor(outBorderColor);
        }
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (currMode == MODE_CIRCLE) {
            ...
            setMeasuredDimension(result+outBorder, result+outBorder);
        } else {
            super.onMeasure(widthMeasureSpec, heightMeasureSpec);
        }
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mDrawMatrix == null && getPaddingTop() == 0 && getPaddingLeft() == 0) {
            ...
            if (currMode == MODE_CIRCLE) {//当为圆形模式的时候
                ...
                canvas.drawCircle(getWidth()/2,getWidth()/2,getWidth()/2,mOuterPaint);

                Bitmap bitmap = drawable2Bitmap(mDrawable);
                mPaint.setShader(new BitmapShader(bitmap, Shader.TileMode.CLAMP, Shader.TileMode.CLAMP));
                canvas.drawCircle(getWidth() / 2, getHeight() /2 , getWidth() / 2-outBorder/2, mPaint);
